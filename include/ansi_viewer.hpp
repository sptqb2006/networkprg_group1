#pragma once
// ANSI escape code renderer for remote nc viewers (streamed over TCP).
...
inline std::string renderFrame(const std::vector<HostState> &hosts,
  ...
  for (const auto &h : hosts) {
    ...
    o << " " << AWHT << name << RST << " ";
    o << pctAnsi(h.cpu, h.name, th, 'c') << makeBar(h.cpu, barW) << " " << b1 << RST << " ";
    o << pctAnsi(h.ram, h.name, th, 'r') << makeBar(h.ram, barW) << " " << b2 << RST << " ";
    o << pctAnsi(h.disk, h.name, th, 'd') << makeBar(h.disk, barW) << " " << b3 << RST << " ";
  ...
  o << ACYN << line('=') << RST << "\n";
  o << AGRY << " Auto-refresh every 2s | Ctrl+C disconnect | Read-only snapshot"
    << RST << "\n";
